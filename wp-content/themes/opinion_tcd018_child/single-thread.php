...
<div id="main_col">
 ...
 <ul id="bread_crumb" class="clearfix">
  ...
  <li><a href="<?php echo get_post_type_archive_link('thread'); ?>"><span><?php _e('スレッド', 'tcd-w'); ?></span></a></li>
 ...
 <div id="left_col">
  ...
  <?php // Recent news --------------------------------------------------------------------------------------- ?>
  <div id="Recent_news">
   <h3 class="headline2"><?php _e("最近のスレッド","tcd-w"); ?></h3>
   <?php
        $args = array('post_type' => 'thread', 'posts_per_page' => 5);
        $recent_thread = get_posts($args);
        if ($recent_thread) {
   ?>
   <ul id="news_list" class="clearfix">
    <?php foreach ($recent_thread as $post) : setup_postdata ($post); ?>
    <li class="clearfix">
